...
function n_elem_detected = process_image(src, ground_truth, teamplates, show_images)
    ...
    check_plate(bw, ground_truth, teamplates);
...
function [n_elem_detected, detected_plate] = check_plate(bw_img, ground_truth, teamplates)
    ...
    for n = 1 : length(teamplates)
        ...
        [max_val, x_peak] = correlate_element(bw_img, part_img);
...
function dst = get_6_max_correlations(correlations)
    ...
    for n = 1 : 6
        ...
        best_val = sorted_values(n);
        dst(best_name) = best_val;
    end
...
%   - X peak
function [max_val, x_peak] = correlate_element(bw_img, teamplate)
    c = normxcorr2(teamplate, bw_img);
    max_val = max(c(:));
    [ypeak, x_peak] = find(c==max_val);
end
